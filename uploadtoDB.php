...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO application (comp_name, contact, email, location, attachment, application_date, type, status, result, to_hire, num_hire)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

    if ($stmt) {
        $type = "p_client";
        $stmt->bind_param("ssssssssssi", $company_name, $contact, $email, $location, $imageData, $date, $type, $status, $result, $to_hire, $manpower);
